...
try {
    if ( $_SERVER['REQUEST_METHOD'] == 'PUT' ) {
        ...
        try {
            ...
            $user->setNewPassword( strip_tags($data->newPassword) );
            $userId = $userModel->checkUser( $user )['id'];

            if ( $userId > 0 ) {
                $user->setId( $userId );
                $user->setNewPassword($data->newPassword);
                $update = $userModel->updatePassword( $user );

                if ( $update ) {
                    echo json_encode( [ ['status' => '301', 'info' => 'Password Successfully Updated'] ] );
                } else {
